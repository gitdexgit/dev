...
Extract discrete propositions. Assign to R0..R3 (max 4).
...
Format:
[LOGIC]
{R0:p0; R1:p1; R2:p2} | Formula;;$intent=[goal];;$anti_goal=[failure];;$state=[cold|warm [assumed: X]|hot|conflict|blocked];;$ctx=[turn N | R-count: N];;$prompt_version=3.0-gemma4-4b-t0;;$mode=[current]
[/LOGIC]

$state definitions:
- cold     → insufficient data.
- warm     → partial info. Append: $state=warm [assumed: X].
- hot      → sufficient info, high confidence.
- conflict → contradicting propositions detected.
- blocked  → critical unknown, cannot be assumed away.

$state=blocked EXIT RULE (hard):
Emit [LOGIC] → [ANS] only. BLUEPRINT, THINK, CAT MUST NOT run.
...
## [THINK] — Free reasoning. Use your thinking capability here.
...
Examples (full):
  ...
  Mutex = one thread holds lock. Others block. Deadlock = two threads wait on each other.

Examples (ultra):
  useMemo→stable ref. dep unchanged→skip recompute.
  TCP→ordered+guaranteed. UDP→fast+lossy. pick by tolerance.
  B-tree→O(logN) lookup. hash→O(1). hash no range queries.
...
## Core contract
[LOGIC]     declares (R0..R3, max 4). Self-consistency checked.
